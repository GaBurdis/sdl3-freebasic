...
If Not SDL_Init(SDL_INIT_VIDEO) Then
    ...
    quit = True
ElseIf Not SDL_CreateWindowAndRenderer("Example Renderer Geometry", WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_RESIZABLE, @win, @renderer) Then
...
While Not quit
    ...
    Dim As Single size = 200.0f + (200.0f * scale)
    ...
    vertices(0).position.x = CSng(WINDOW_WIDTH) / 2.0f
    vertices(0).position.y = (CSng(WINDOW_HEIGHT) - size) / 2.0f
    vertices(0).color.r = 1.0f
    vertices(0).color.a = 1.0f
    vertices(1).position.x = (CSng(WINDOW_WIDTH) + size) / 2.0f
    vertices(1).position.y = (CSng(WINDOW_HEIGHT) + size) / 2.0f
    vertices(1).color.g = 1.0f
    vertices(1).color.a = 1.0f
    vertices(2).position.x = (CSng(WINDOW_WIDTH) - size) / 2.0f
    vertices(2).position.y = (CSng(WINDOW_HEIGHT) + size) / 2.0f
    ...
    vertices(3).color.g = 1.0f
    vertices(3).color.b = 1.0f
